...
import com.paragon.dictionary.fbreader.higo.readbook.ui.fragment.BookSettingFragment;
import com.paragon.dictionary.fbreader.higo.readbook.ui.fragment.SettingFragmentStatePagerAdapter;
...
public final class FBReader extends FBReaderMainActivity implements ZLApplicationWindow, BookSettingFragment.OnFragmentInteractionListener {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        myMainView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        x2 = event.getX();
                        y2 = event.getY();
//                        Log.i("AAA", x2 + "-" + x1 + "=" + Math.abs(x2 - x1));
    ...
    private void initBookSetting() {
        ...
        ArrayList<SettingItem> screenList = new ArrayList<SettingItem>();

        List<MenuNode> menuNodes = MenuData.topLevelNodes();
        for (MenuNode n : menuNodes) {
            if (n instanceof MenuNode.Item) {
                Integer iconId = ((MenuNode.Item) n).IconId;
                String actionId = ((MenuNode.Item) n).Code;
                Log.i("AAA", "actionId=" + actionId);
                SettingItem settingItem = new SettingItem();

                settingItem.setAction(actionId);
                String name = ZLResource.resource("menu").getResource(actionId).getValue();

                settingItem.setName(name);
                if (iconId != null) {
                    settingItem.setIcon(iconId);
                }
                list.add(settingItem);

                Log.i("AAA", name);
            } else if (n instanceof MenuNode.Submenu) {
                Log.i("AAA", "----" + ((MenuNode.Submenu) n).Code);
                Integer iconId = null;
                MenuNode.Submenu submenu = (MenuNode.Submenu) n;
                String id = submenu.Code;
                String name = ZLResource.resource("menu").getResource(id).getValue();

                SettingItem settingItem = new SettingItem();
                settingItem.setAction(id);
                settingItem.setName(name);
                if (iconId != null) {
                    settingItem.setIcon(iconId);
                }
                ...
                ArrayList<MenuNode> children = submenu.Children;
                for (MenuNode n2 : children) {
                    if (n2 instanceof MenuNode.Item) {
                        Integer iconId2 = ((MenuNode.Item) n2).IconId;
                        String actionId2 = ((MenuNode.Item) n2).Code;
                        Log.i("AAA2", "actionId2=" + actionId2);
                        SettingItem settingItem2 = new SettingItem();

                        settingItem2.setAction(actionId2);
                        String name2 = ZLResource.resource("menu").getResource(actionId2).getValue();

                        settingItem2.setName(name2);
                        if (iconId2 != null) {
                            settingItem2.setIcon(iconId2);
                        }
                        screenList.add(settingItem2);
        ...
        int page = (int) (list.size() / 8);
        for (int i = 0; i < page; i++) {
            for (int j = 8 * i; j < 8 * (i + 1); j++) {
                datas.add(list.get(j));
            }
            fragments.add(BookSettingFragment.newInstance(datas, myFBReaderApp,screenList));
            datas.clear();
        }
        if (list.size() % 8 != 0) {
